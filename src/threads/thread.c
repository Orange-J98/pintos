...
/* Initializes the threading system by transforming the code
   ...
   通过将当前正在运行的代码转换为线程来初始化线程系统。 通常这是行不通的，
   但是在这种情况下有可能实现，仅是因为loader.S小心地将堆栈底部放在页面边界。 
   ...
   thread_create().
   调用此函数后，请确保在尝试使用thread_create（）创建任何线程之前初始化页面分配器。

   It is not safe to call thread_current() until this function
   finishes. 
   在此函数完成之前调用thread_current()是不安全的
   */
                    ...
                    thread_func *function, void *aux)
{
  ...
  /*如果新建的线程优先级比原线程高，则重新调用原线程，保证高优先级的线程可以先运行*/
...
/* Transitions a blocked thread T to the ready-to-run state.
   ...
   update other data. 
   将阻塞的线程T转换为准备运行状态。如果T没有进入阻塞状态，就会产生一个错误。
  （使用thread_yield（）使正在运行的线程进入就绪状态。)
   此函数不会抢占正在运行的线程。 这很重要：如果调用者自己已禁用了中断，则可能
   希望它可以自动取消阻塞线程并更新其他数据。*/
...
/*放弃CPU，当前线程不会进入休眠状态，可能会被调度程序恰好立刻再次调度*/
...
将当前线程的优先级设置成为一个新的优先级new_priority
*/
...
/* Idle thread.  Executes when no other thread is ready to run.
   ...
   线程空闲。 当没有其他线程准备运行时执行。空闲线程最初由thread_start（）
   放置在就绪列表中。 它最初将被调度一次，这时它将初始化idle_thread，传递给
   它的信号量“ up”以使thread_start（）能够继续进行，并立即阻塞。 在那之后，
   空闲线程将永远不会出现在就绪列表中。 当就绪列表为空时，next_thread_to_run（）
   作为特殊情况返回它。*/
static void
...
/*这个函数是一个list_less_func，用来比较a和b中哪一个的aux值更小，如果a小，则返回true，否则返回false*/
...
/* Chooses and returns the next thread to be scheduled.  Should
   ...
   idle_thread. 
   选择并返回要调度的下一个线程。 应该从运行队列返回线程，除非运行队列为空的。
   （如果正在运行的线程可以继续运行，则它将在运行队列中。）如果运行队列为空，
   则返回idle_thread
   注意! 当run queue 为空的时候，返回idle_thread。
   */
